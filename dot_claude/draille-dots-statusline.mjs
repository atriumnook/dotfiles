...
const T50 = '\x1b[38;2;68;78;99m';   // モデル名・cwd  — de-emphasized
const T35 = '\x1b[38;2;36;44;61m';   // セパレータ │

// Branch accents (git workflow semantics, high-chroma, dark-theme)
const BR_MAIN    = '\x1b[38;2;226;230;240m'; // main/master — neutral, no alert
const BR_DEV     = '\x1b[38;2;100;181;246m'; // develop/dev — calm blue
const BR_FEAT    = '\x1b[38;2;38;222;129m';  // feat/*      — vivid green (growth)
const BR_FIX     = '\x1b[38;2;255;145;77m';  // fix/hotfix  — warning orange
const BR_RELEASE = '\x1b[38;2;179;136;255m'; // release/*   — ceremonial purple

// Gradient stops: traffic-light semantics
const G_LO  = [46, 230, 130];  // #2EE682 vivid green
const G_MID = [255, 214, 0];   // #FFD600 pure yellow
const G_HI  = [255, 82, 82];   // #FF5252 vivid red
...
process.stdin.on('end', () => {
  ...
  const branch = cwd ? git('git rev-parse --abbrev-ref HEAD', cwd) : '';
  ...
  const unstaged = parseDiffStat(git('git diff --shortstat', cwd));
  const staged   = parseDiffStat(git('git diff --cached --shortstat', cwd));
