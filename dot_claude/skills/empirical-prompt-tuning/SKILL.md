...
description: agent 向けテキスト指示（skill / slash command / task プロンプト / CLAUDE.md 節 / コード生成プロンプト）の品質を実測で検証・改善したいとき。プロンプトや skill を新規作成・大幅改訂した直後、またはエージェントの挙動が期待通りにならない原因を指示側の曖昧さに求めたいときに使う。
---
...
- `retrospective` — タスク後の学び固定化。本 skill はプロンプト開発中、retrospective はタスク終了後、と使い分ける
